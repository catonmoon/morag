...
- **Retry с экспоненциальным backoff** — настраиваемое количество повторных попыток; LLM использует встроенный retry openai, HTTP-эмбеддеры — собственную политику (задержка и backoff фиксированы: 1s × 2.0); Confluence и Jira — встроенный `urllib3.Retry`
...
#     username: user
#     password: pass
#     max_retries: 3              # retry при сетевых ошибках (urllib3); 0 = без retry
#     attachments:
#       enabled: true             # обработка PDF-вложений (требует docling)
#   jira:
#     url: https://jira.example.com
#     username: user
#     password: pass
#     max_retries: 3              # retry при сетевых ошибках (urllib3); 0 = без retry
#     custom_fields:              # кастомные поля (названия подтянутся из Jira API)
#       - customfield_10100
#       - customfield_10200
...
| Jira | `attached_jira` | Задачи, обнаруженные по ссылкам в других документах; описание, комментарии, подзадачи, кастомные поля |
...
индексируются автоматически. Поддерживаются кастомные поля: список ID (`custom_fields` в конфиге),
названия полей загружаются автоматически из Jira API.
