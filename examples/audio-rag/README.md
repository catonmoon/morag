...
- **Транскрайб-бэкенд** — `services/asr-adaptor` (диаризация + двухпроходный Whisper с
  ...
  primary-деплой нативный на Mac (`services/asr-adaptor/deploy/mac`, ставится одной командой);
...
⚠️ **Настройки голосов задайте до первого прогона** — подробности, два типовых профиля с числами
и способ проверить результат в разделе
[«Первая настройка корпуса: голоса»](../../services/asr-adaptor/deploy/mac/README.md#первая-настройка-корпуса-голоса).
На записи «один докладчик + вопросы из зала» дефолты сводят всех участников в одного `Speaker_N`.
