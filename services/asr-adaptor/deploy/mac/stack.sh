...
# голосов — живёт в git репозитория проекта, а секреты остаются в env-файле машины. Читается ПОСЛЕ
# него и потому перекрывает: профиль отвечает за «что за материал», env-файл — за «чем и куда ходить».
