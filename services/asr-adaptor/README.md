...
`ASR_MATCH_THRESHOLD` (0.55) — косинус, с которого голос считается знакомым;
`ASR_MIN_GUEST_MIN` (2.0) — сколько минут неузнанный голос должен наговорить, чтобы стать
ОТДЕЛЬНЫМ человеком; короче — приклеится к ближайшему независимо от косинуса. Порог доменный:
для записей с короткими репликами из зала (митап, планёрка) ставить ~0.25, иначе все участники
сведутся в одного `Speaker_N` — молча, расшифровка будет выглядеть удавшейся;
`ASR_PROMPT_BUDGET` (≤224 токена подсказки Whisper),
