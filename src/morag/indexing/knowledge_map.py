...
# TODO(robustness, 2026-06): на ПУСТОМ входе ({children_summaries}/{text} пусты) LLM копирует
# few-shot ПРИМЕРЫ ниже прямо в ответ (кейс: season2 с пустыми doc_summary из-за 402 → KM выдал
# «подкаст про Kubernetes/SRE»). Фикс: не звать LLM на пустом входе (узел без summary → скип/пусто).
# Примеры ниже обезличены: были реальные корп-названия — нельзя в публичном репо.
...
- Имена систем, сервисов, инструментов (например: Kubernetes, ClickHouse, PostgreSQL, Redis).
