...
Если удаление предыдущего чанка делает текущий чанк менее понятным — объедини их.

Входной текст может быть:
- частью документа
- целым разделом документа
- заранее подготовленным блоком

Если входной текст уже выглядит как один логически завершённый блок (например, заранее выделенная часть таблицы или списка):
...
- Не объединяй несколько строк таблицы в один чанк
...
| Model | Max batch size | GPU required |
|------|---------------|--------------|
| resnet50 | 32 | Yes |
| bert-base | 8 | Yes |
...
{
  "chunks": [
    "Модель resnet50 поддерживает максимальный размер батча 32 и требует GPU.",
    "Модель bert-base поддерживает максимальный размер батча 8 и требует GPU."
  ]
}
