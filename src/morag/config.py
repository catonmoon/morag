...
class RetrievalFeaturesConfig(BaseModel):
    ...
    # Nudge «поиск перед ответом»: если агент финалит ход БЕЗ единого tool-вызова
    # (типовой провал на follow-up вопросах в диалоге: отвечает из истории →
    # ноль источников → мёртвые [N]), одноразово впрыскивается требование сделать
    # search по сути вопроса. Механика как diversity-nudge. Off по умолчанию.
    require_search_before_answer: bool = False
...
class RetrievalPromptsConfig(BaseModel):
    ...
    # Оверрайд ФИНАЛЬНОЙ инструкции (последний user-месседж перед ответом,
    # `_stream_final`). Пусто → зашитый документный стандарт («кратко, 3-5 абзацев,
    # структурируй таблицами») байт-в-байт. Задано → заменяет обёртку целиком
    # (таблица цитат [N] по-прежнему добавляется кодом после). Нужно корпусам,
    # где документный стандарт вредит (разговорный пересказ аудио-транскриптов).
