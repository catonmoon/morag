...
# Методика «### 2. ВЫПОЛНЕНИЕ» — как пользоваться core-тулами (search/find_section/
# get_doc): несколько search под грани, язык/словарь запроса, section_ids vs doc_ids,
# глубокое чтение, шум при широком поиске. Живёт в слое тулов (а не в монолите
# _SYSTEM_PROMPT) — pipeline вставляет в {tool_methodology}. Optional-тулы могут
# дополнять методику через свой ToolSpec.prompt_section (клеится следом).
